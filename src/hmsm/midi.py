...
def create_midi(note_start: List[float], note_length: List[float], midi_note: List[int]) -> mido.MidiFile:
    ...
    # TODO: Refactor this to make useful calculations about time conversion in the final midi file
    ...
    events = [[events[n][0], round(events[n][1] - events[n - 1][1]) if n > 0 else round(events[n][1]), events[n][2]] for n in range(0, len(events))]
